...
function bindUI() {
  ...
  $('#chat-start-engine').addEventListener('click', async () => {
    ...
    if (m) { await startEngine('text', m, { ctx: +$('#chat-ctx').value || 8192, nCpuMoe: +$('#chat-moe').value || 0, noMmap: $('#chat-nommap').checked, mlock: $('#chat-mlock').checked, directIo: $('#chat-dio').checked }); refreshEngineStatus(); }
  });

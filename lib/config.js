...
const defaults = {
  ...
  harness: { enabled: true, keepAlive: true, model: '' },
  audio: { outputDir: `${home}/PolarisAudio`, copyTranscript: false },
  ui: { theme: 'auto', fontFamily: 'inter', fontSize: 14, advancedThinking: false },
  remotes: [],
  engines: {
    text: { binary: `${home}/llama.cpp/build/bin/llama-server`, port: 8080, ngl: 99, ctx: 8192, maxTokens: 8192, threads: 4, threadsBatch: 4, batchSize: 2048, ubatchSize: 512, flashAttn: 'on', parallel: 4, contBatching: true, selfMtp: false, draftModel: '', draftMin: 1, draftMax: 3, draftPMin: 0.0, nCpuMoe: 0, noMmap: false, mlock: false, directIo: false, cacheTypeK: 'f16', cacheTypeV: 'f16', topP: 0.95, topK: 40, minP: 0.05, repeatPenalty: 1.1, extraArgs: [], provider: 'local', activeRemoteId: '' },
...
for (const k of Object.keys(defaults)) {
  ...
  else if (defaults[k] && typeof defaults[k] === 'object' && !Array.isArray(defaults[k]) && config[k] && typeof config[k] === 'object' && !Array.isArray(config[k])) {
    for (const sk of Object.keys(defaults[k])) if (config[k][sk] === undefined) config[k][sk] = defaults[k][sk];
  }
}
for (const k of Object.keys(defaults.engines)) {
  if (!config.engines[k]) config.engines[k] = defaults.engines[k];
  else { // fill missing sub-keys without wiping user values
    for (const sk of Object.keys(defaults.engines[k])) if (config.engines[k][sk] === undefined) config.engines[k][sk] = defaults.engines[k][sk];
  }
}

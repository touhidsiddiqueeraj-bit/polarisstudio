...
function mmprojFor(modelPath) {
  try {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      if (best && !name.includes(base)) continue; // prefer a projector named after this model
...
class Engine {
  ...
  argsFor(modelPath, opts) {
    ...
    if (this.type === 'text') {
      ...
      if (kv !== 'f16') a.push('--cache-type-k', kv, '--cache-type-v', opts.cacheTypeV ?? this.cfg.cacheTypeV ?? kv);
  ...
  async start(modelPath, opts = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      if (await this._spawn(modelPath, opts)) return this.status();

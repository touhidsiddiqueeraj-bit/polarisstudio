...
class Download extends EventEmitter {
  ...
  async start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on('error', (e) => { if (!this.aborted) reject(e); });
      const req = https.get(target.url, { headers, timeout: 30000 }, (res) => {
        if (res.statusCode === 416) {
          stream.end(() => {
            if (stat.size > 0) {
              try { fs.renameSync(this.part, this.dest); } catch (e) { return reject(e); }
            }
      ...
      req.on('error', (e) => { stream.destroy(); if (!this.aborted) reject(e); });
    });
  }
...
function followRedirects(url, headers, hops = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, { headers, method: 'HEAD', timeout: 20000 }, (res) => {
      res.resume();
